...
// Clean output dir
glob
    .sync(path.resolve(__dirname, "../resources/css/*.s[a|c]ss"))
    .forEach((file) => {
        fs.rmSync(file);
    });
...
ImportTree.prototype.parseImports = function () {
    ...
    if (fileContent && fileContent.length > 0) {
        ...
        this.fileContent.forEach(function (line, index) {
...
ImportTree.prototype._resolveImportPath = function (importPath) {
    ...
    if (importPath.charAt(0) === "~") {
        resolvedPath = path.resolve(
            __dirname,
            "../node_modules/" + importPath.substr(1)
        );
    } else {
        resolvedPath = path.resolve(path.dirname(this.filename), importPath);
    }
...
ImportTree.prototype.toString = function () {
    if (this.imports.length > 0) {
        ...
        this.imports.forEach(function (importEntry) {
            ...
            self.fileContent[importEntry.line] =
                prefix + importEntry.tree.toString() + affix;
        });
    }
...
glob
    .sync(path.resolve(__dirname, "../resources/scss/*.scss"))
    .forEach((file) => {
