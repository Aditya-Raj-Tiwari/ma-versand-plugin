...
import SearchTooltip from "./SearchTooltip.vue";
export default {
  name: "item-search",
  components: {
    SearchTooltip,
  ...
  mounted() {
    ...
    this.$nextTick(() => {
      ...
      this.$refs.searchInput.value = !isNullOrUndefined(urlParams.query)
        ? urlParams.query
        : "";
    });
  },
  ...
  methods: {
    search() {
      if (this.$refs.searchInput.value.length) {
        if (pathnameEquals(App.urls.search)) {
          ...
          window.open(
            `${App.urls.search}?query=${this.searchString}`,
            "_self",
            false
          );
        }
    ...
    onBlurSearchField(event) {
      ...
      if (
        ...
        (!isNullOrUndefined(target) && !this.$el.contains(target))
      ) {
